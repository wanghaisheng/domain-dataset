...
def insert_into_db(data, batch_size=10, use_d1=True, db_path="pricing_data.sqlite"):
    ...
    if use_d1:
        ...
        insert_into_sqlite(data, batch_size, db_path)

def insert_into_sqlite(data, batch_size, db_path="pricing_data.sqlite"):
    """Insert rows into a SQLite database with hash checks and batch inserts."""
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    if not data:
    ...
    for i in range(0, len(data), batch_size):
        ...
        for row in batch:
            ...
            escaped_values = (
                ...
                sqlite3.Connection('').execute("SELECT quote(?)", (row['domain'],)).fetchone()[0],
                sqlite3.Connection('').execute("SELECT quote(?)", (row['lang'],)).fetchone()[0],
                sqlite3.Connection('').execute("SELECT quote(?)", (row['currency'],)).fetchone()[0],
                sqlite3.Connection('').execute("SELECT quote(?)", (row['links'],)).fetchone()[0],
                sqlite3.Connection('').execute("SELECT quote(?)", (row['prices'],)).fetchone()[0],
                 sqlite3.Connection('').execute("SELECT quote(?)", (row['priceplans'],)).fetchone()[0],
                 sqlite3.Connection('').execute("SELECT quote(?)", (row['htmlpath'],)).fetchone()[0],
                 sqlite3.Connection('').execute("SELECT quote(?)", (row['mdpath'],)).fetchone()[0],
            )
        ...
        try:
              ...
              conn.commit()
              print(f"Inserted batch {i // batch_size + 1} to SQLite successfully.")
...
async def main():
    ...
    parser.add_argument(
        "--dbtype", type=str, choices=["d1", "sqlite"], default="d1",
        help="Database type to use: 'd1' for Cloudflare D1 or 'sqlite' for local SQLite"
    )
    parser.add_argument(
